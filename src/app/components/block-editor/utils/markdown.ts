...
export function markdownToHtml(markdown: string, blockType?: string): string {
  ...
  if (blockType === "to_do_list") {
    ...
    const todoMatch = normalized.match(/^\[([ xX])\]\s*(.*)$/);
    const isChecked = todoMatch?.[1].toLowerCase() === "x";
    const content = todoMatch?.[2] ?? "";
    const contentHtml = marked.parseInline(content) as string;

    return `<ul data-type="taskList"><li data-type="taskItem" data-checked="${isChecked}"><label><input type="checkbox" ${
      isChecked ? "checked" : ""
    }><span></span></label><div><p>${contentHtml}</p></div></li></ul>`;
  }
