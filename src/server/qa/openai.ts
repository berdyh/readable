...
function getOpenAiConfig(): OpenAiConfig {
  const apiKey = requireEnvVar(
    'OPENAI_API_KEY',
    'to answer questions. Set it in your environment.',
  );
  const baseUrl = requireEnvVar(
    'OPENAI_API_BASE_URL',
    'to answer questions. Update .env.local if needed.',
  );
  const model = requireEnvVar(
    'OPENAI_QA_MODEL',
    'for the QA endpoint. Update .env.local if needed.',
  );
...
export interface OpenAiQaParams {
  systemPrompt: string;
  userPrompt: string;
