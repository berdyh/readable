...
function getDefaultModel(taskType?: string): string {
  ...
  return (
    ...
    (taskType === 'summary' || taskType === 'summarize'
      ? process.env.ANTHROPIC_SUMMARY_MODEL ?? 'claude-3-5-sonnet-20241022'
      : process.env.ANTHROPIC_QA_MODEL ?? 'claude-3-5-sonnet-20241022')
  );
}
...
export class AnthropicProvider implements LlmProviderInterface {
  ...
  async generateJson(
    ...
    try {
      ...
      const payload = (await response.json限期 as Record<string, unknown>;
