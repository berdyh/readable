...
export async function POST(request: NextRequest) {
  ...
  try {
    const result = await getSelectionCitations(payload.paperId, payload.selection);

    return NextResponse.json(result, { status: 200 });
  } catch (error) {
    console.error("[editor] Failed to fetch selection citations", error);
    const message = error instanceof Error ? error.message : "Unable to fetch citations.";
