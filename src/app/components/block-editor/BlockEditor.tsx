...
function BlockEditorContent({
  ...
  useEffect(() => {
    const handler = (event: Event) => {
      ...
      const dispatchNavigationResult = (
        status: "success" | "unavailable",
        reason?: "target-not-found" | "element-not-found",
      ) => {
        if (!detail.requestId) {
          return;
        }
        ...
        window.dispatchEvent(
          new CustomEvent("block-editor-navigate-result", {
            detail: {
      ...
      if (targetBlock) {
        ...
        if (blockElement) {
          ...
          if (blockElement instanceof HTMLElement) {
            const previousTabIndex = blockElement.getAttribute("tabindex");
            ...
            window.setTimeout(() => {
              if (previousTabIndex === null) {
                ...
                blockElement.setAttribute("tabindex", previousTabIndex);
              }
            }, 2000);
          }
          ...
          dispatchNavigationResult("success");
        } else {
          dispatchNavigationResult("unavailable", "element-not-found");
        }
      } else {
        dispatchNavigationResult("unavailable", "target-not-found");
      }
    };
